...
3.  **Reminder Creation Protocol (Strict)**:
    ...
    - MUST set `sessionTarget: "isolated"`.
    - MUST set `payload.kind: "agentTurn"`.
    - MUST set `payload.deliver: true`.
    - MUST set `payload.channel: "whatsapp"`.
    - MUST set `payload.message` to the reminder text.
    - **Example payload**:
      ```json
      {
        "name": "Reminder",
        "schedule": { "kind": "at", "atMs": <future_timestamp_ms> },
        "sessionTarget": "isolated",
        "payload": {
          "kind": "agentTurn",
          "message": "Time to check the logs!",
          "deliver": true,
          "channel": "whatsapp"
        }
      }
      ```
    - After creation, always confirm the reminder by listing it via `cron.list`.
...
- **REFER TO "Core Directives" ABOVE FOR MANDATORY CRON PARAMETERS**
